...
final class DefinitionMaterializationSchemaProvider implements IMaterializationSchemaProvider {
	...
	private function loadManifests(): array {
		...
		foreach($datasets as $name => $dataset) {
			if(!is_array($dataset)) {
				throw new \RuntimeException('Materialization setting must be an array: ' . (string)$name);
			}
			...
			$data = $dataset['definition'] ?? null;
			if(!is_array($data)) {
				throw new \RuntimeException('Materialization setting must contain a definition array: ' . (string)$name);
			}
	...
	private function isDatasetEnabled(array $dataset): bool {
		return !array_key_exists('enabled', $dataset) || $this->normalizeBoolean($dataset['enabled'], true);
	}
	...
	private function validateManifest(MaterializationManifest $manifest, string $name): void {
		if($manifest->id === '') {
			throw new \RuntimeException('Materialization setting must define a non-empty id: ' . $name);
		}

		if($manifest->logicalTable === '') {
			throw new \RuntimeException('Materialization setting must define target.logicalTable or logicalTable: ' . $name);
		}
	}
